...
agents.each do |agent|
  ...
  on(agent, "cat #{fs_file}") do |res|
    fail_test "didn't find the mount #{name}" unless res.stdout.include? name.to_s
  end

  step 'verify entry is mounted'
  on(agent, 'mount') do |res|
    fail_test "didn't find the mount #{name} mounted" unless res.stdout.include? name.to_s
  end
end
